  ...
  do
   ...
   if [ $COUNTER -eq "$ext" ]
    then
         ...
         #Sig ggHH
         #sed -i 's/0.512345/'${xsgghhh}'/g' ws_SM_c2v01_noCosTheta_run2_ggHHNLO_KL${ext}/Node_SM/*.txt
         #Sig VBFHH
         sed -i 's/0.554321/'${xsvbfhh}'/g' ws_run2_ggHHNLO_MVAggHHLoose_KL${ext}/Node_SM/*.txt
  ...
  do
   ...
   if [ $COUNTER2 -eq "$ext" ]
    then
         ...
         #Sig ggHH
         sed -i 's/0.512345/'${xsgghhh}'/g' ws_run2_ggHHNLO_MVAggHHLoose_KL${ext}/Node_SM/*.txt
    fi 
  done <"$file2"
done
