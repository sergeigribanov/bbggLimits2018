...
do
  ...
  sed -e 's/\(C2\)\([0-9][0-9]\)/\1'${ext}'/g' run_extLimit_tempC2.sh >run_extLimitC2${ext}.sh
  sed -e 's/\(C2\)\([0-9][0-9]\)/\1'${ext}'/g' condor_job_runLimit_tempC2.sub >condor_job_runLimitC2${ext}.sub
  #. runWS.sh
  condor_submit condor_job_runLimitC2${ext}.sub 
done
